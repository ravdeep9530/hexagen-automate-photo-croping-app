...
describe('image-utils', () => {
  beforeEach(() => {
    ...
    sharpPipeline.metadata.mockResolvedValue({ width: 1200, height: 1600 });
  ...
  it('validates crop parameters against image bounds and requested aspect ratio', async () => {
    const { validateCropParameters } = await import('../../../src/lib/image-utils');
    ...
    ).toEqual({ valid: false, message: 'Crop coordinates must be greater than or equal to 0' });
    ...
    ).toEqual({ valid: false, message: 'Crop aspect ratio does not match the requested aspect ratio' });
  });

  it('returns image dimensions from sharp metadata', async () => {
    const { getImageDimensions } = await import('../../../src/lib/image-utils');

    await expect(getImageDimensions(Buffer.from('original-image'))).resolves.toEqual({ width: 1200, height: 1600 });
    expect(sharpMock).toHaveBeenCalledWith(Buffer.from('original-image'));
    expect(sharpPipeline.metadata).toHaveBeenCalledTimes(1);
  });
