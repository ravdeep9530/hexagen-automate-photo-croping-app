...
export const PhotoEditorPage = ({
  ...
  const logging = useLogging({ userId });
  ...
  const uploadZoneApiClient = useMemo(
    () => ({
      uploadImage: async (request: {
        ...
        const requestId = logging.createRequestId();
        const startedAt = Date.now();
        const result = await apiClient.uploadImage(request);

        if (result.error) {
          logging.logFlowEvent({
            event: "upload",
            status: "error",
            requestId,
        ...
        logging.logFlowEvent({
          ...
          status: "success",
          requestId,
          userId: result.data.image.userId,
  ...
  const downloadPanelApiClient = useMemo(
    () => ({
      downloadCroppedPhoto: async (request: { photoId: string }) => {
        ...
        const startedAt = Date.now();
        const relatedPhoto =
          store.getState().processedPhotos.find((photo) => photo.id === request.photoId) ?? null;
        ...
        if (result.error) {
          logging.logFlowEvent({
            ...
            status: "error",
            requestId,
            imageId: relatedPhoto?.imageId ?? null,
            startedAt,
            metadata: {
  ...
  const handleImageUploaded = async (imageId: string) => {
    ...
    if (!result.data.isValid) {
      logging.logFlowEvent({
        ...
        startedAt,
        metadata: {
          validation_failure_count: result.data.failures.length,
        },
      });
