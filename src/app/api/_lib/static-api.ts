...
export function methodNotAllowed(allowedMethods: readonly string[] = ['GET']): NextResponse<ApiErrorPayload> {
  return NextResponse.json(
    apiError(
      'METHOD_NOT_ALLOWED',
      `Method not allowed. These static metadata endpoints only support ${allowedMethods.join(', ')}.`
    ),
    {
      status: 405,
      headers: {
...
export const rejectUnsupportedStaticApiMethod = methodNotAllowed;
