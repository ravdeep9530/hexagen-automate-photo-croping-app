...
export function asApiRouteError(
  error: unknown,
  fallback: { status: number; code: string; message: string; details?: string | null },
): ApiRouteError {
  if (error instanceof ApiRouteError) {
    return error;
  }

  return createApiError(fallback.status, fallback.code, fallback.message, fallback.details);
}
