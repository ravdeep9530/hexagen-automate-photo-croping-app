...
export function PhotoUpload() {
  ...
  const [croppedPhoto, setCroppedPhoto] = useState<CroppedPhotoResult | null>(null);
  ...
  const uploadFile = useCallback((file: File) => {
    ...
    xhr.onload = () => {
      ...
      if (xhr.status >= 200 && xhr.status < 300) {
        const nextUploadedPhotoUrl = typeof response.uploadedPhotoUrl === 'string' ? response.uploadedPhotoUrl : null;
        const nextImageId = typeof response.imageId === 'string' ? response.imageId : file.name;

        setStatus('success');
        setProgress(100);
        setUploadedPhotoUrl(nextUploadedPhotoUrl);
        setUploadedImageId(nextImageId);
        setIsCropperOpen(Boolean(nextUploadedPhotoUrl));
        return;
      }
  ...
  return (
    <>
        ...
        <div aria-live="polite" className="mt-4 space-y-2 text-sm">
          ...
          {status === 'success' ? (
            <div>
              ...
              <p className="text-slate-700">Crop your image before submitting it for passport review.</p>
              <button
                aria-label="Open cropper"
                className="mt-3 rounded-md border border-slate-900 px-4 py-2 text-sm font-medium text-slate-900"
                disabled={!uploadedPhotoUrl || !uploadedImageId}
                onClick={() => {
          ...
          {croppedPhoto ? (
            <div>
              <p className="font-medium text-green-700">Cropped photo saved.</p>
              <a className="text-blue-700 underline" href={croppedPhoto.url}>
                {croppedPhoto.url}
              </a>
            </div>
          ) : null}
      ...
      {uploadedPhotoUrl && uploadedImageId ? (
        <CropperModal
