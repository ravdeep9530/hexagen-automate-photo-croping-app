...
export const calculatorShellClasses: CalculatorShellClasses = Object.freeze({
  root: [
    'relative isolate min-h-screen overflow-hidden bg-slate-100 px-4 py-6 text-slate-950',
    'transition-colors duration-300 sm:px-6 sm:py-10 lg:px-8',
  ...
  background: [
    ...
    'before:absolute before:left-1/2 before:top-10 before:h-72 before:w-72 before:-translate-x-1/2 before:rounded-full before:bg-fuchsia-300/20 before:blur-3xl',
  ...
  eyebrow: 'text-sm font-semibold uppercase tracking-[0.24em] text-indigo-600 dark:text-indigo-300',
  title: 'mt-2 text-3xl font-bold tracking-tight text-slate-950 sm:text-5xl dark:text-white',
  description: 'mt-3 max-w-2xl text-base leading-7 text-slate-600 dark:text-slate-300',
  toolbar: 'flex shrink-0 items-center justify-start sm:justify-end',
  layout: 'grid w-full min-w-0 grid-cols-1 items-start gap-4 lg:grid-cols-[minmax(0,1fr)_22rem] xl:grid-cols-[minmax(0,1fr)_24rem]',
  calculatorCard: [
    'w-full min-w-0 rounded-[2rem] border border-white/35 bg-white/55 p-3',
    'shadow-2xl shadow-slate-900/10 backdrop-blur-2xl sm:p-4 lg:p-5',
    'dark:border-white/10 dark:bg-slate-950/45 dark:shadow-black/30',
  ].join(' '),
  calculatorStack: 'flex min-w-0 flex-col gap-3 sm:gap-4',
  historyColumn: 'min-w-0 lg:sticky lg:top-6',
  footerHint: 'text-center text-xs font-medium text-slate-500 dark:text-slate-400',
});
...
export function createCalculatorShellViewModel(
  ...
  return {
    ...
    keypad: createCalculatorKeypadViewModel({ compact: options.compactKeypad }),
    historyPanel: createHistoryPanelViewModel(calculator.history, options.history),
    ...
    sections: createCalculatorShellSections(),
    classes: {
...
function createCalculatorShellSections(): CalculatorShellViewModel['sections'] {
  return {
    display: {
      component: 'CalculatorDisplay',
      className: 'min-w-0',
      motion: calculatorShellSectionMotion,
    },
    keypad: {
