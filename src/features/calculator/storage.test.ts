...
describe('calculator history storage', () => {
  ...
  it('discards invalid JSON and invalid history entry shapes safely', () => {
    const malformedEntries: unknown[] = [
      null,
      '10 + 5 = 15',
      { expression: '10 + 5 =', result: '15', createdAt: iso(1_700_000_000_000) },
      { id: '', expression: '10 + 5 =', result: '15', createdAt: iso(1_700_000_000_000) },
      { id: 'history-1', expression: '', result: '15', createdAt: iso(1_700_000_000_000) },
      { id: 'history-1', expression: '10 + 5 =', result: '', createdAt: iso(1_700_000_000_000) },
      { id: 'history-1', expression: '10 + 5 =', result: '15', createdAt: Number.NaN },
      { id: 'history-1', expression: '10 + 5 =', result: '15', createdAt: 'today' },
      { id: 'history-1', expression: '10 + 5 =', result: '15', createdAt: '2024-01-01' },
    ];
  ...
  it('accepts entries with string fields and ISO createdAt values only', () => {
  ...
  it('caps history to the latest 20 entries in newest-first order', () => {
    ...
    expect(normalized.map((historyEntry) => Date.parse(historyEntry.createdAt))).toEqual(
  ...
  it('exposes addEntry, clearHistory, and setHistory controller behavior', () => {
    ...
    expect(first).toEqual([entryWithExpression('first', '1 + 1 =', '2', iso(100))]);
    expect(controller.history).toEqual(first);

    const set = controller.setHistory([
      entry('older', 1),
      { id: 'invalid', expression: '', result: '0', createdAt: iso(2) },
      entry('newer', 2),
    ]);
  ...
  it('creates missing ids and ISO timestamps for new calculator history entries', () => {
  ...
  it('normalizes numeric and Date timestamps to ISO strings for new entries', () => {
    expect(createHistoryEntry({
      expression: '1 + 1 =',
      result: '2',
      createdAt: 1_700_000_000_000,
    }).createdAt).toBe('2023-11-14T22:13:20.000Z');

    expect(createHistoryEntry({
      expression: '2 + 2 =',
      result: '4',
      createdAt: new Date(1_700_000_000_001),
    }).createdAt).toBe('2023-11-14T22:13:20.001Z');
  });
});
